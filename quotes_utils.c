...
void	replace_global_var(t_cmd *cmd, char *var)
{
	...
	open_simple = look_for_open('\'', '\"', cmd->original, cmd->dc);
	open_double = look_for_open('\"', '\'', cmd->original, cmd->dc);
	close_simple = look_for_closure('\'', '$', cmd->original, cmd->dc);
	double_close = look_for_closure('\"', '$', cmd->original, cmd->dc);
	if (!open_simple && !open_double && !close_simple && !double_close)
		cmd->dollar_value[cmd->d_counter++] = search_value(var, cmd);
			...
			|| (countleft(cmd->original, cmd->dc, '\'', '\'') % 2 == 0
				|| (countleft(cmd->original, cmd->dc, '\'', '\'') % 2 != 0
					&& countleft(cmd->original, cmd->dc, '\"', '\"') % 2 == 0)))
